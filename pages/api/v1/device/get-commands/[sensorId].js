/**
 * @swagger
 * \api\v1\device\get-commands\{sensor_id}:
 *   get:
 *     summary: Get sensor's commands
 *     parameters:
 *       - in: path
 *         name: sensor_id
 *     tags:
 *       - Device Endpoints
 *     description: Get sensor's commands
 *     responses:
 *      200:
 *         description: Success
 *      400:
	...
	protectDeviceRoute(req, res)
		.then(async (result) => {
			...
			if (result.statusCode === 401) {
				...
				try {
					const commands = await Sensor.findById(req.query.sensorId).select(
						'commands'
					);
