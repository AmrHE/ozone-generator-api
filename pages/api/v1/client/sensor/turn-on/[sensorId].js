/**
 * @swagger
 * \api\v1\client\sensor\turn-on\{sensor_id}:
 *   get:
 *     security:
 *       - authorization: []
 *     summary: Turn the sensor's command to on
 *     parameters:
 *       - in: path
 *         name: sensor_id
 *     tags:
 *       - Client Endpoints
 *     description: Turn the sensor's command to on
 *     responses:
 *      200:
 *         description: Success
 *      400:
			...
			if (result.statusCode === 401) {
				...
				try {
					const sensor = await Sensor.findByIdAndUpdate(
						req.query.sensorId,
						{ commands: { command: 1, createdAt: Date.now() } },
						{
							new: true,
							runValidators: true,
						}
